...
from databases import AnswerLog, PrecomputedSessionAnswer, ScoringPromptLog, SessionUserMap, AgentSettings, ChatLog, Scores, SessionInteractionLog
    ...
    async def score_item(self, session_id: str, conversation_context: str, criterion: dict, db: Session, precomputed_data: Optional[PrecomputedSessionAnswer]):
        ...
        try:
            if item_type == "RAG":
                ...
                score = await self._check_non_term(session_id, conversation_context, criterion, db)
            elif item_type == "RAG_emo_response":
                score = await self._check_emo_response(session_id, conversation_context, criterion, db)
            elif item_type == "RAG_review_med_history":
                score = await self._check_review_med_history(session_id, conversation_context, criterion, db)
            elif item_type == "first_med_mix_method":
                # 對應: 說明保可淨使用方式 (泡製)
                score = await self._check_med_mix_method(session_id, conversation_context, criterion, db)
            elif item_type == "first_med_mix_time":
                # 對應: 說明樂可舒使用方式 (錠劑時間)
                score = await self._check_first_med_mix_time(session_id, conversation_context, criterion, db)
            elif item_type == "hydration_method":
                # [修正] 區分 s1 (2000cc) 與 s2 (1000cc)
                score = await self._check_hydration_method(session_id, conversation_context, criterion, db)
            elif item_type == "ideal_intestinal":
                score = await self._check_ideal_intestinal(session_id, conversation_context, criterion, db)
            elif item_type == "RAG_explain_npo":
                # 這是單純檢查「有無提到禁水」，不同於 RAG_confirm_npo (檢查時間點準確性)
                score = await self._check_explain_npo_mention(session_id, conversation_context, criterion, db)
...
#12/3 從non_term開始修正 好的 從non_term跳過XD 我這邊沒有那個專業用語集
    async def _check_non_term(self, session_id: str, conversation_context: str, criterion: dict, db: Session) -> int:
        """
        ...
        return await self._call_llm_and_log(session_id, criterion['id'], prompt, SCORING_MODEL_NAME, db)
    # 1207 修正
    async def _check_emo_response(self, session_id: str, conversation_context: str, criterion: dict, db: Session) -> int:
    ...
    # 1207 修正
    async def _check_review_med_history(self, session_id: str, conversation_context: str, criterion: dict, db: Session) -> int:
        """檢查學員是否詢問過往用藥經驗。"""
        prompt = f"""
    ...
    #1207 修正中
    async def _check_hydration_method(self, session_id: str, conversation_context: str, criterion: dict, db: Session) -> int:
        item_id = criterion['id']
        ...
        符合輸出 "1"，不符合或未提及輸出 "0"。
        """
        return await self._call_llm_and_log(session_id, item_id, prompt, SCORING_MODEL_NAME, db)

    async def _check_ideal_intestinal(self, session_id: str, conversation_context: str, criterion: dict, db: Session) -> int:
        """檢查是否說明清腸理想狀態 (淡黃清澈)。"""
        prompt = f"""
    ...
    async def _check_explain_npo_mention(self, session_id: str, conversation_context: str, criterion: dict, db: Session) -> int:
        """
    ...
    def _check_organization_sequence_by_time(self, session_id: str, has_dulcolax: bool, has_special_meds: bool, db: Session) -> float:
        """
        ...
        logs = db.query(AnswerLog.scoring_item_id, AnswerLog.created_at).filter(
            AnswerLog.session_id == session_id,
            AnswerLog.score == 1,
        ...
        if not (t_diet < t_powder < t_npo):
            logger.info(f"[{session_id}] 組織效率扣分: 基礎順序錯誤 (Diet->Powder->NPO)")
            return 1.0
        ...
        if has_special_meds:
            ...
            if not (t_npo < t_special):
                logger.info(f"[{session_id}] 組織效率扣分: 特殊用藥順序錯誤 (應在禁水後)")
                return 1.0
    ...
    async def calculate_final_scores(self, session_id: str, db: Session) -> Dict[str, str]:
        ...
        if agent_settings and agent_settings.med_code:
            ...
            if agent_settings.med_code.strip() and agent_settings.med_code.strip() != "無":
        ...
        interaction_log = db.query(SessionInteractionLog).filter(SessionInteractionLog.session_id == session_id).first()
        
        ui_review_score = 0.0
        if interaction_log:
            # 依據 PDF 權重計算
            if interaction_log.viewed_alltimes_ci: ui_review_score += 2.0 # 歷次清腸
            if interaction_log.viewed_chiachi_med: ui_review_score += 3.0 # 本院用藥
            if interaction_log.viewed_med_allergy: ui_review_score += 1.0 # 過敏史
            if interaction_log.viewed_disease_diag: ui_review_score += 1.0 # 疾病診斷
            if interaction_log.viewed_cloud_med:   ui_review_score += 2.0 # 雲端藥歷
            
        logger.info(f"[{session_id}] UI Interaction Score Calculated: {ui_review_score}")
        ...
        scores["medical_interview_score"] = (
            val_hello + val_sit + proper_guidance_total + val_confirm_self + 
            val_ask_exp + val_no_term + val_emo
        )
        ...
        val_purpose = (is_passed('explain_med_purpose.s1') or is_passed('explain_med_purpose.s2')) * 0.5
        ...
        if has_dulcolax: # 組合 F
             # PDF: "說明藥物使用時機及方式(+2)"。
             # 這裡簡單處理：各佔 1 分
             val_med_method = is_passed('med_mix_method_and_time.s1') * 1.0 + is_passed('med_mix_method_and_time.s2') * 1.0
        else: # 組合 E
             # 只有保可淨，佔 2 分
        ...
        val_hydro = (is_passed('hydration_and_goal.s1') or is_passed('hydration_and_goal.s2')) * 1.0
        ...
        scores["counseling_edu_score"] = (
            val_purpose + val_note_time + val_med_method + val_hydro + 
            val_ideal + val_onset + val_pain_check + val_npo_explain + 
            val_special_med + val_diet
        )
        ...
        scores["humanitarian_score"] = score_respect + val_satisfy + score_empathy + val_trust + val_comfort
        ...
        if interaction_log:
            # 依據 PDF 權重計算
            if interaction_log.viewed_alltimes_ci: rmh_3_sub += 2.0 # 歷次清腸
            if interaction_log.viewed_chiachi_med: rmh_3_sub += 3.0 # 本院用藥
            if interaction_log.viewed_cloud_med:   rmh_3_sub += 2.0 # 雲端藥歷
        ...
        scores["clinical_judgment_score"] = (
            val_has_special + val_judge_time1 + val_judge_time2 + 
            val_judge_npo + val_judge_stop + val_judge_understand + val_judge_reasonable
        )
        ...
        scores["overall_clinical_skills_score"] = val_attitude + val_integration + val_effectiveness
