...
async def perform_precomputation(session_id: str, agent_code: str, db: Session):
    ...
    check_time_obj = datetime.strptime(f"{exam_date.strftime('%Y-%m-%d')} {time_part}", '%Y-%m-%d %H:%M')
    ...
    db.commit()
    logger.info(f"[{session_id}] Precomputed answers saved successfully for module {MODULE_ID}.")