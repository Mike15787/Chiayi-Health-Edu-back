...
class ChatLogFeedbackModel(BaseModel):
    id: int
    role: str
    text: str
    time: datetime
...
class AchievedItemModel(BaseModel):
    ...
    description: str = Field(..., alias="item_description")
...
class FeedbackMessageModel(BaseModel):
    ...
    text: str
    time: datetime
    achieved_items: List[AchievedItemModel] = [] # 預設為空列表

class AttributionFeedbackModel(BaseModel):
    chat_log: ChatLogFeedbackModel
    achieved_items: List[AchievedItemModel]

# 修改 FullFeedbackResponse 來使用新的統一模型
...
async def get_full_feedback(session_id: str, db: Session = Depends(get_db)):
    ...
    # 2. 獲取所有歸因紀錄，並按 chat_log_id 整理成字典
    ...
    for attr in attributions:
        ...
        item_details = scoring_criteria_map.get(attr.scoring_item_id, {"item": "未知評分項"})
        attribution_map[attr.chat_log_id].append(
            AchievedItemModel(item_id=attr.scoring_item_id, item_description=item_details['item'])
        )
    ...
    for log in chat_logs:
        # 無論角色為何，都創建一個 FeedbackMessageModel
        # 如果是 user，就從 map 中獲取 achieved_items，否則為空
        achieved_items = attribution_map.get(log.id, [])
        
        full_history.append(
            FeedbackMessageModel(
                id=log.id,
                role=log.role, # 'user' or 'patient'
                text=log.text,
                time=log.time,
