...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "http://127.0.0.1:8080",
        "http://127.0.0.1:5173",
        "https://0c521f9bb2a0.ngrok-free.app  ",
    ],
...
async def init_models():
    ...
    try:
        ...
        # 篩選繁體中文語音
        chinese_voices = [
            v for v in voices if "zh-TW" in v["Locale"] or "zh-CN" in v["Locale"]
        ]
        ...
        for voice in available_voices:
            logger.info(
                f"  - {voice['Name']}: {voice['ShortName']} ({voice['Gender']})"
            )
    ...
    async def summarize_conversation(self, session_id: str):
        ...
        try:
            ...
            if summary and "抱歉" not in summary:
                ...
                # 1. 將生成的總結印到終端機
                logger.info(
                    f"--- [DEBUG] Generated Summary for Session {session_id} ---"
                )
                logger.info(summary)
                logger.info("---------------------------------------------------------")
...
def transcribe_audio(audio_path: str) -> dict:
    ...
    prompt_text = (
        "以下是關於大腸鏡衛教的醫學對話。關鍵字包含:大腸鏡檢查、腸鏡、清腸劑、清腸藥、保可淨"
        "無痛檢查、一般檢查、禁水、糞水、淡黃色、清澈"
...
def get_agent_settings_as_dict(agent_code: str) -> Optional[dict]:
    ...
    try:
        ...
        if agent_settings_obj:
            ...
            return {
                c.key: getattr(agent_settings_obj, c.key)
                for c in agent_settings_obj.__table__.columns
            }
        return None
    finally:
        db.close()
...
async def root():
    ...
    return {
        ...
        "status": "healthy",
        "models": {"edge_tts": True, "whisper": whisper_model is not None},
...
async def speech_to_text(
    ...
    try:
        ...
        session_map = (
            SessionLocal()
            .query(SessionUserMap)
            .filter(SessionUserMap.session_id == current_session_id)
            .first()
        )
        if not session_map:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Session {current_session_id} not found. Please create a session first.",
            )
        ...
        if user_text:
            ...
            # 觸發背景評分
            asyncio.create_task(
                score_utterance_task(user_message_log.id, current_session_id)
            )
...
async def speech_to_text_chunk(audio: UploadFile = File(...)):
    ...
    try:
        ...
        loop = asyncio.get_event_loop()
        transcription = await loop.run_in_executor(
            executor, transcribe_audio, temp_path
        )
...
async def score_utterance_task(chat_log_id: int, session_id: str):
    ...
    try:
        ...
        if newly_passed_item_ids:
            ...
            db_task.commit()
            logger.info(
                f"[{session_id}] Attributed chat_log {chat_log_id} to items: {newly_passed_item_ids}"
            )

    except Exception as e:
        logger.error(
            f"Background scoring task failed for chat_log_id {chat_log_id}: {e}"
        )
        logger.error(traceback.format_exc())
        db_task.rollback()
    finally:
...
async def chat_api(req: ChatRequest):
    ...
    try:
        ...
        if not agent_settings:
            raise HTTPException(
                status_code=404, detail=f"找不到 agent_code: {agent_code} 的设定"
            )

        history = await conversation_manager.get_formatted_history(session_id, limit=15)
        precomputed_data_obj = (
            SessionLocal()
            .query(PrecomputedSessionAnswer)
            .filter(PrecomputedSessionAnswer.session_id == session_id)
            .first()
        )
        precomputed_data_dict = (
            {
                c.key: getattr(precomputed_data_obj, c.key)
                for c in precomputed_data_obj.__table__.columns
            }
            if precomputed_data_obj
            else None
        )

        patient_agent_builder = module_manager.get_patient_agent_builder(module_id)
        prompt = patient_agent_builder(
            user_text, history, agent_settings, precomputed_data_dict
        )

        module_config = module_manager.get_module_config(module_id)
        llm_response = await generate_llm_response(
            prompt, model_name=module_config.PATIENT_AGENT_MODEL_NAME
        )
        ...
        return ChatResponse(
            text=llm_response, audioUrl=audio_url, session_id=session_id
        )
...
async def end_chat_session(
    ...
    try:
        ...
        return {
            "status": "ended",
            "message": "Session marked as completed and interaction logged.",
        }
...
async def health_check():
    ...
    return {
        ...
        "models": {"edge_tts": True, "whisper_initialized": whisper_model is not None},
        "directories": {
...
async def agent_info_find(
    edu_type: str = Query(
        ...,
        description="衛教類型: 清腸劑衛教, Warfarin抗凝衛教, DOAC抗凝衛教, 慢性病衛教",
    ),
    chemical_type: str = Query(
        ..., description="藥物種類: 保可淨(BowKlean), 腸見淨(Klean-prep)等"
    ),
    ...
    try:
        ...
        logger.info(
            f"查詢 AgentSettings: level={level}, agent_code={agent_code}, username={username}"
        )
        ...
        if not agent_setting:
            raise HTTPException(
                status_code=404, detail=f"找不到 agent_code: {agent_code} 的資料"
            )
...
async def create_session(session_info: SessionInfo, db: Session = Depends(get_db)):
    ...
    try:
        ...
        existing_session = (
            db.query(SessionUserMap)
            .filter(SessionUserMap.session_id == session_info.session_id)
            .first()
        )

        if existing_session:
            logger.info(
                f"Session {session_info.session_id} 已存在，模組: {existing_session.module_id}"
            )
            return {
                "message": "Session already exists",
                "session_id": session_info.session_id,
                "module_id": existing_session.module_id,
            }
        ...
        if session_info.module_id not in module_manager.modules:
            raise HTTPException(
                status_code=400,
                detail=f"指定的模組 '{session_info.module_id}' 不存在或未載入。",
            )
        ...
        precomputation_func = module_manager.get_precomputation_performer(
            session_info.module_id
        )
        await precomputation_func(session_info.session_id, session_info.agent_code, db)

        logger.info(
            f"创建新 session: {session_info.session_id}, 模組: {session_info.module_id}"
        )
